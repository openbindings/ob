...
func WireErrorFrom(err *openbindings.InvocationError) *WireError {
	if err == nil {
		return &WireError{Code: openbindings.ErrCodeRuntime, Message: "unknown error"}
	}
	return &WireError{Code: err.Code, Message: err.Message, Details: err.Details}
}
...
func (e *WireError) InvocationError() *openbindings.InvocationError {
	if e == nil {
		return &openbindings.InvocationError{Code: openbindings.ErrCodeRuntime, Message: "unknown error"}
	}
	return &openbindings.InvocationError{Code: e.Code, Message: e.Message, Details: e.Details}
}
