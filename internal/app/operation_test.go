...
func TestOperationAliasAdd_Basic(t *testing.T) {
	...
	result, err := OperationAliasAdd(obiPath, "acme.fetch", []string{"openbindings.document-store.get"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if len(op.Aliases) != 1 || op.Aliases[0] != "openbindings.document-store.get" {
...
func TestOperationAliasAdd_Collision(t *testing.T) {
	dir := t.TempDir()
	obiPath := writeInterface(t, dir, "test.obi.json", minimalInterface(map[string]any{
		"acme.fetch": map[string]any{"aliases": []string{"openbindings.document-store.get"}},
		"acme.other": map[string]any{},
	}))

	// Adding an alias already claimed by another operation must fail (flat namespace).
	_, err := OperationAliasAdd(obiPath, "acme.other", []string{"openbindings.document-store.get"})
	if err == nil {
...
func TestOperationAliasAdd_ResolveByAlias(t *testing.T) {
	...
	result, err := OperationAliasAdd(obiPath, "openbindings.document-store.get", []string{"x.y.z"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestOperationAdd_WithAliases(t *testing.T) {
	...
	_, err := OperationAdd(OperationAddInput{
		OBIPath: obiPath,
		Key:     "acme.set",
		Aliases: []string{"openbindings.document-store.set"},
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestOperationAdd_KeyCollidesWithAlias(t *testing.T) {
	...
	_, err := OperationAdd(OperationAddInput{OBIPath: obiPath, Key: "openbindings.document-store.get"})
	if err == nil {
...
func TestOperationRename_IntoExistingAlias(t *testing.T) {
	...
	// invalid document.
	_, err := OperationRename(obiPath, "acme.other", "openbindings.document-store.get")
	if err == nil {
