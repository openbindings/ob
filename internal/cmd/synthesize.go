...
openbindings.openapi@3:-). A stdin artifact is content, not a location:
...
Examples:
  ...
  curl -s https://api.example.com/openapi.json | ob synthesize openbindings.openapi@3:- -o api.obi.json
  ob synthesize --input '{"sources":[{"bindingSpec":"openbindings.openapi@3","location":"api.yaml"}]}'`,
