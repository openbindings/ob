...
openbindings.openapi@4:-). A stdin artifact is content, not a location:
...
Examples:
  ...
  curl -s https://api.example.com/openapi.json | ob synthesize openbindings.openapi@4:- -o api.obi.json
  ob synthesize --input '{"sources":[{"bindingSpec":"openbindings.openapi@4","location":"api.yaml"}]}'`,
