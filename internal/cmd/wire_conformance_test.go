	...
	for _, schemaName := range []string{"OperationInvokerOutputFrame", "BindingInvokerOutputFrame"} {
		...
		for _, tc := range cases {
			t.Run(schemaName+"/"+tc.name, func(t *testing.T) {
				...
				if _, present := wireErr["category"]; present {
					t.Fatalf("emitted error frame carries a closed category: %s", raw)
				}
				if _, present := wireErr["effects"]; present {
					t.Fatalf("emitted error frame carries retry effects: %s", raw)
				}
			})
		}
	}
