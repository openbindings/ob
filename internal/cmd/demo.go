...
func printBanner(port, grpcPort int) {
	...
	fmt.Fprintln(w)
	fmt.Fprintf(w, "  %s\n\n", s(dim).Render("GraphQL needs its executable document — an interpretation point, so it is context (set once, standing):"))
	fmt.Fprintf(w, "  $ ob context set %s/graphql \\\n", base)
	fmt.Fprintf(w, "      --value '{\"configuration\":{\"document\":\"query { getMenu { items { name description category sizes { id label price } } } }\"}}'\n")
