...
`ob source add` accepts a bare path (`./openapi.json`) and auto-detects the current binding revision, or an explicit binding-spec identifier (`openbindings.openapi@6:./openapi.json`). `ob source pull` reads the source, extracts operations and schemas, and writes the operations and their bindings back into the OBI.
...
Sources (2)
  openapi           openbindings.openapi@6   ./openapi.json       out of sync (synced 3d ago, ob 0.2.0)
...
```bash
ob source add interface.json openbindings.openapi@6:./api.yaml          # embeds by default
ob source add interface.json 'openbindings.openapi@6:https://example.com/api.yaml?embed'  # fetch and pin a remote artifact
```
...
```bash
ob source add interface.json openbindings.openapi@6:https://example.com/api.yaml
```

To keep a local working file but publish a pointer, pair `--resolve location` with `--uri`:

```bash
ob source add interface.json openbindings.openapi@6:./api.yaml --resolve location --uri https://cdn.example.com/api.yaml
```
...
curl -X POST http://localhost:20290/bindings/prepare \
  ...
  -d '{
    "source": { "bindingSpec": "openbindings.openapi@6", "location": "https://api.example.com/openapi.json" },
