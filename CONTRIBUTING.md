...
The web-based experience is provided by [Panjir](https://panjir.com), which connects to a locally running `ob start` instance. Interface browsing, execution, and workspace management happen through Panjir's web UI powered by the ob start HTTP API.
...
- **`internal/server/`** — HTTP server infrastructure for `ob start`.
